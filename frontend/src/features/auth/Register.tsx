...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!acceptedTerms) {
      setError(
        language === "th" ? "กรุณายอมรับเงื่อนไข" : "Please accept terms"
      );
      return;
    }
    try {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-main-gradient">
      <div className="w-full max-w-md animate-in fade-in slide-in-from-bottom-4 duration-500">
        <button
          ...
          className="flex items-center gap-2 text-white/80 hover:text-white mb-6 transition-all font-medium"
        >
          <ArrowLeft size={18} />{" "}
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl p-10">
          ...
          {error && (
            <div className="mb-6 p-3 bg-red-50 text-red-500 text-xs rounded-xl text-center border border-red-100">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-start gap-3 px-1">
              <input
                ...
                className="mt-1 w-4 h-4 rounded border-gray-300 text-[#00A699] focus:ring-[#00A699]"
                id="terms"
              />
              <label
                htmlFor="terms"
            ...
            <button
              type="submit"
              className="w-full bg-[#FF6B4A] hover:bg-[#ff5232] text-white py-4 rounded-2xl font-bold text-lg shadow-lg shadow-orange-100 transition-all transform hover:scale-[1.02]"
            >
              {t.registerBtn}
            </button>
          </form>
