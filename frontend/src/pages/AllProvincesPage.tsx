...
export default function AllProvincesPage({
  language = "th",
}: AllProvincesPageProps) {
  ...
  useEffect(() => {
    const fetchProvinces = async () => {
      ...
      setErrorMsg(null);
      try {
        // แก้ไข: ใช้ endpoint ที่ถูกต้อง /tours/provinces
        const response = await fetch(`${API_URL}/tours/provinces`);
  ...
  const filteredProvinces = provinces.filter((province) => {
    ...
    const matchRegion =
      selectedRegion === "all" || provinceRegion === selectedRegion;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8 text-center">
        ...
        <div className="max-w-xl mx-auto relative mb-8">
          ...
          <input
            ...
            placeholder={
              language === "th" ? "ค้นหาชื่อจังหวัด..." : "Search provinces..."
            }
        ...
        {/* Expandable Region Filter */}
        <div className="flex flex-row items-center justify-center w-full max-w-5xl mx-auto h-12 relative">
          <button
            onClick={() => setIsFilterExpanded(!isFilterExpanded)}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 z-20 shrink-0 ${
              isFilterExpanded || selectedRegion !== "all"
          ...
          >
            ...
            {!isFilterExpanded && (
              <span
                className={`ml-1 ${
                  selectedRegion !== "all" ? "font-bold" : "font-normal"
                }`}
              >
                {currentRegionName}
              </span>
          ...
          <div
            className={`flex items-center overflow-hidden transition-all duration-500 ease-in-out relative ${
              isFilterExpanded
                ? "max-w-[1000px] opacity-100 ml-3"
            ...
            >
              {REGIONS.map((region) => (
                <button
                  ...
                  className={`px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-300 shrink-0 backdrop-blur-xl hover:-translate-y-0.5 active:scale-95 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
        {loading ? (
          ...
          <div className="text-center py-20 bg-red-50 rounded-3xl border border-red-100 shadow-sm text-red-600">
            ...
            <p>
              กรุณาตรวจสอบว่า Backend (NestJS) เปิดอยู่หรือไม่ (Error:{" "}
              {errorMsg})
            </p>
          </div>
          ...
          <div className="text-center py-20 bg-white rounded-3xl border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-500">
              {language === "th"
                ? "ทดลองเปลี่ยนคำค้นหา หรือเลือกภูมิภาคอื่นดูสิ"
                : "Try changing your search term or selecting a different region."}
            </p>
          </div>
        )}
      </div>
