...
export function UserProfilePage({ language, onNavigate }: UserProfilePageProps) {
  ...
  // ✅ โหลด profile จาก API ตอน mount เพื่อให้ได้ phone จาก DB จริง
  useEffect(() => {
    const loadProfile = async () => {
      try {
        if (userService?.getProfile) {
          ...
          // sync ลง localStorage เพื่อให้ autofill ใช้ได้แม้ offline
  ...
  const handleSaveProfile = async () => {
    ...
    const applyToUI = (name: string, phone: string) => {
      setDisplayFullName(name);
      setDisplayPhone(phone);
      // ✅ sync localStorage ทั้ง 2 key ที่ autofill ใช้
      try {
        const storedUser = JSON.parse(localStorage.getItem("user") || "{}");
        storedUser.fullName = name;
        storedUser.phone    = phone;
    ...
    try {
      ...
      applyToUI(editForm.fullName, editForm.phone); // fallback: update UI แม้ API fail
    } finally {
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="px-6 pb-6">
              <button onClick={() => setShowNotifModal(false)} className="w-full bg-[#00A699] hover:bg-[#008c81] text-white font-bold py-3.5 rounded-xl transition-all active:scale-[0.98] flex items-center justify-center gap-2">
